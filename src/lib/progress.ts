 ...
 * 解析の各回が、解析全体に占める割合。
 *
 * 解析はファイルを2回読む。3回目(リミッターの効きを見る試し処理)は
 * **必要なときだけ**で、会話中心の録音では多くの場合省かれる。
 *
 * 以前はここを 1/3 ずつにしていた。省かれた回では 2/3 まで進んだところで
 * いきなり 1 まで飛ぶ。60分の回で実測すると、全体のバーが 13% → 21% と
 * 跳ねていた。3回目のぶんを小さく取っておけば、省かれても跳ねは小さく、
 * 走ったときも止まって見えない。
 */
export const ANALYZE_SHARE = [0.45, 0.45, 0.1] as const;
