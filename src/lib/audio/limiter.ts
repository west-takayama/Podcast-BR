 ...
 *
 * ここで抑えた値がそのまま届くわけではない。**MP3 にすると膨らむ**。
 * 実測(scripts の音質検査)では、上限 -1.2dBFS で書き出した MP3 を
 * 復号すると 0.12〜0.23dB 高くなっていて、真のピークは -0.97 dBTP、
 * つまり -1 dBTP を**超えていた**。0.2dB の余裕では足りない。
 *
 * -1.5 でも -1.12 dBTP まで来た回があり、余裕は 0.1dB しかなかった。
 * -2.0 なら実測 -1.69〜-1.85 dBTP で、0.7dB の余裕がある。
 *
 * 下げても小さくは聞こえない。仕上がりの音量は後段のラウドネス調整
 * (-19/-16 LUFS)で決まるためで、実際どちらの上限でも仕上がりは
 * -19.0 LUFS ちょうどに着地している(実測)。増えるのは瞬間的な山を
 * 抑える量だけ。
 */
export const CEILING_DBFS = -2.0;
